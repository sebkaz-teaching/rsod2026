...
### 🔹 Cztery litery ACID — każda osobno

| Litera | Właściwość | Co gwarantuje |
|--------|------------|----------------|
| **A** | Atomicity (atomowość) | Transakcja wykonuje się w całości albo wcale — dokładnie przykład z Kontami niżej: albo obie aktualizacje sald przejdą, albo żadna. |
| **C** | Consistency (spójność) | Transakcja przenosi bazę z jednego poprawnego stanu w drugi — więzy integralności (`CHECK`, `FOREIGN KEY`, `UNIQUE`) muszą być spełnione i przed, i po. Jeśli transakcja złamałaby którykolwiek, baza ją odrzuci. |
| **I** | Isolation (izolacja) | Transakcje wykonywane równocześnie nie "widzą" swoich niedokończonych zmian nawzajem — tyle, ile dokładnie "nie widzą", określa **poziom izolacji** (sekcja 3 niżej). |
| **D** | Durability (trwałość) | Po `COMMIT` zmiana przetrwa — nawet awarię zasilania zaraz potem. Baza zapisuje to na trwałym nośniku (dziennik/WAL), zanim potwierdzi sukces klientowi. |
...
Bazy danych zapewniają różne poziomy izolacji, które równoważą spójność danych i
wydajność — im silniejsza izolacja, tym więcej blokad i tym wolniej działa system
przy dużej współbieżności. Standard SQL definiuje cztery poziomy, każdy zapobiegający
kolejnej "anomalii":

| Poziom izolacji | Odczyt "brudnych" danych (dirty read) | Powtórny odczyt daje inny wynik (non-repeatable read) | Nowe wiersze "pojawiają się" w kolejnym odczycie (phantom read) |
|---|---|---|---|
| **READ UNCOMMITTED** | ❌ możliwy | ❌ możliwy | ❌ możliwy |
| **READ COMMITTED** | ✅ zablokowany | ❌ możliwy | ❌ możliwy |
| **REPEATABLE READ** | ✅ zablokowany | ✅ zablokowany | ❌ możliwy* |
| **SERIALIZABLE** | ✅ zablokowany | ✅ zablokowany | ✅ zablokowany |

\* W PostgreSQL `REPEATABLE READ` w praktyce blokuje też phantom read (dzięki
implementacji przez migawkę danych — *snapshot isolation*) — to mocniejsza
gwarancja, niż wymaga sam standard SQL. Nie zakładajcie tego zachowania w innych
silnikach bez sprawdzenia dokumentacji.

- **Dirty read** — transakcja A czyta dane, które transakcja B jeszcze nie
  zacommitowała (i może zaraz wycofać przez `ROLLBACK`) — A oparła decyzję na
  danych, które "nigdy się nie wydarzyły".
- **Non-repeatable read** — transakcja A czyta ten sam wiersz dwa razy w trakcie
  swojego działania i dostaje **różne** wartości, bo transakcja B zdążyła go
  zmienić i zacommitować pomiędzy odczytami A.
- **Phantom read** — transakcja A wykonuje ten sam warunek `WHERE` dwa razy i za
  drugim razem dostaje **inny zestaw wierszy** (nie zmienioną wartość istniejącego
  wiersza, tylko zupełnie nowy/zniknięty wiersz), bo B wstawiła/usunęła coś w
  międzyczasie.
...
➡️ Na poziomie REPEATABLE READ oba SELECT-y zobaczą ten sam zestaw danych, nawet jeśli inna transakcja je zmieniła w międzyczasie — to właśnie ochrona przed non-repeatable read z tabeli wyżej.

::: {.callout-note}
## Kompromis, nie "im wyżej, tym lepiej"
`SERIALIZABLE` daje najsilniejsze gwarancje, ale kosztem wydajności (więcej blokad,
więcej transakcji odrzuconych do ponowienia przy konflikcie). Domyślny poziom w
PostgreSQL to `READ COMMITTED` — świadomy wybór "wystarczająco bezpiecznie dla
większości aplikacji, bez kosztu SERIALIZABLE". Podnosicie poziom izolacji tylko
tam, gdzie konkretna anomalia z tabeli wyżej realnie zagraża Waszym danym.
:::
